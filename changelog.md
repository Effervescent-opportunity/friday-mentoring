...
- Оставила AuthEventRepository публичным для интеграционных тестов
- Оставила свои ужасные названия, ведь дальше будут еще правки, а придумывать названия я буду еще час
- Возможно, в AuthEventReader нужно возвращать не AuthEventEntity, а какой-нибудь AuthEventDto, но я не уверена
- 
интеграционные тесты падают при поднятом докер компоуз
мой вариант закрытия стрима не очень - если есть хоть одна битая строчка, то отправки не будет, стрим будет вытаскивать ее первым делом и падать
todo попробовать завтра проставить тем строкам фолс и запустить еще раз. потом остановить всё и попробовать еще раз. кто решает
в каком порядке будет выдача из стрима? почему в psql поднялись эти записи наверх?

может быть придется попросить посмотреть с закомменченными тестами, если не буду успевать, чтобы не переделывать 100500 раз. хз
