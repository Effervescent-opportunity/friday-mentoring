...
- Добавлено шифрование паролей с помощью bcrypt, потому что он был первым в списке вариантов. Причем с наименьшей strength для быстродействия
- Убраны ограничения доступа в `ClockService`, потому что они переехали на уровень выше
- Пользователи, роли и зашифрованные пароли теперь хранятся в БД
- Добавлены пользователи `wakeup` (роль TIME) и `beza` (роль SECURITY). У пользователя `root` также есть роль ADMIN. У всех пароль = `password`
- Возможность сменить пароль не добавлена - не получилось прикрутить шифрование к JdbcUserDetailsManager

### Существующие ограничения и проблемы

1. Зашифрованные пароли можно расшифровать одной командой, такой способ шифрования не является надежным
2. Можно изменять данные пользователей и их права в БД, нужно, чтобы доступ к БД был только у "доверенных лиц"
3. Нет возможности изменить пароль, нет срока действия пароля
4. Названия ролей и пользователей захардкожены в коде

## Девятое задание

- В `compose.yml` добавлены ограничения на CPU и память
- Отправка в Logstash запустится, если задан его адрес
...
работу Spring Security - работу фильтров, запросы аутентификации и авторизации), Kafka Producer, Logback (количество логов каждого типа)

## Десятое задание

- Swagger UI находится по адресу: https://localhost:8443/swagger-ui/index.html#
- Добавлен индекс для поля `event_time`
