...
- Для "мелких" и "крупных" тестов генерятся отдельные `*.exec` файлы в `build/jacoco`, задача `jacocoTestReport` сделает общий html-отчет 
...
- Задача `allTestsReport` сделает общий html-отчет по запуску тестов в `build/reports/allTests/index.html`
- Задача `jacocoTestCoverageVerification` упадет, если покрытие тестами каждого класса (кроме всех entity и Application) меньше 80%
