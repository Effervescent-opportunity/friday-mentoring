...
- Есть предположение, что Zookeeper в моем случае с 1 нодой всего не нужен, оно не проверялось
...
корректной работы сервиса, таких событий будет больше 90%, и в этом нет смысла
- Сериализация в JSON, потому что, кажется, это самый используемый способ
...
времени разобраться, как проверить, что доступ есть, не хватило. Тест, соответственно, только на случай "доступ к Кафке есть"
