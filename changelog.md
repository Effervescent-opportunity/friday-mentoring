...
- Основная проблема с юнит тестами - их скучно писать даже мне, любящей закопаться в незначительные детали
- Тестовая реализация интерфейса мне видится нужной крайне редко, куда проще замокать всё
- По поводу холивара для создания интерфейса с одной реализацией, в конкретно моем случае есть смысл сделать такой интерфейс для отправки 
...
может понадобиться доработать все, пора бы уже привыкнуть к этому). И тогда идея делать интерфейс для уменьшения связности (?), закрепления контракта, тестирования
  (IDD интересная вещь, а в тестах так и не поняла), и намечания планов - хороша. Но да, вообще не на все. Подумать, что на работе можно было бы
сделать интерфейсами? //todo rewrite lalala
- Оценка покрытия и реалистичности проверки такими тестами:
Можно написать нереалистичные тесты, например подавая на вход метода (например com.friday.mentoring.service.AuthEventService.processEvent) null, чего не может произойти при запуске приложения
- Сперва я думала сделать не интеграционные тесты с `@DataJpaTest` и базой h2, но тогда придется копировать скрипт миграции и исправлять его под синтаксис h2. 
Плюс неизвестно, какие дальше функции базы и репозиториев понадобятся, так что проще заложиться на более сложное, чем нынешние CRUD, которые можно протестировать на h2.
Это, конечно, не идеальный интеграционный тест, но как есть. (может еще и не заработает, там спринг бут  тест над базовыми интеграционным)
