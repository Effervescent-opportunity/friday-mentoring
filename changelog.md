...
- Выделение интерфейса не было сделано, просто нет части теста в KafkaProducerTest для случая работающей Кафки, потому что не хочу множить сущности без большой необходимости
- Тестовая реализация интерфейса мне видится нужной крайне редко, куда проще замокать всё или написать интеграционный тест
- Тесты с `@DataJpaTest` в лежат в `testInt` и используют Testcontainers. Сперва я думала сделать не интеграционные тесты с `@DataJpaTest` и базой h2, но тогда придется копировать
  скрипт миграции и исправлять его под синтаксис h2. Плюс неизвестно, какие дальше функции базы и репозиториев понадобятся, так что проще заложиться на 
  более сложное, чем нынешние CRUD, которые можно протестировать на h2. Это, конечно, не идеальный интеграционный тест, но как есть.
...
- Сейчас у меня покрытие юнит тестами для `service` - 88%, вместе с интеграционными - 98%, 
