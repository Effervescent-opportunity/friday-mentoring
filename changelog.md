...
- У решения есть риски:
1. При обновлении Spring'а типы в его AuditApplicationEvent могут перестать совпадать с `AuthEventType`
2. Если в `auth_event` есть некорректная строчка, то стрим каждый раз будет падать на ней и не отправлять дальше корректные 
(порядок элементов в стриме у меня не меняется без перезапуска приложения)
