...
- Для проверки доступности Кафки создан бин AdminClient. Тогда в логах при старте приложения 2 adminclient'а, но без прямого создания бина приложение не поднимается, не могу получить доступ к единственному AdminClient, второй - дефолтный выключается сам хз почему
