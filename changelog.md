...
разделения зависимостей. Был еще вариент с test suite, но не получилось понять, что это и зачем, если можно без него
...
- Для "мелких" и "крупных" тестов генерятся отдельные `*.exec` файлы в `build/jacoco`, таска jacocoTestReport сделает общий html-отчет 
по покрытию в `build/reports/jacoco/test/html/index.html`
- Таска `allTestsReport` сделает общий html-отчет по запуску тестов в `build/reports/allTests/index.html`
- какая от кого зависит и почему (и проверить!)
- Есть очень странная проблема - при запуске задачи `build` при выполнении integrationTest в лог пишется строк 20 консольного вывода тестов, хотя не должно
- LALALA что такое test suite, я так и не поняла
